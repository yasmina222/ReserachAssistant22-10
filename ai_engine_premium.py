...
"""Premium research engine using Serper + GPT-4o-mini"""
def __init__(self):
    ...
    # COST OPTIMIZATION: Changed from gpt-4-turbo-preview to gpt-4o-mini
    # Reduces cost by 98% while maintaining excellent JSON extraction quality
...
def research_school(self, school_name: str, location: Optional[str] = None) -> Dict[str, Any]:
    """Complete school research using search + GPT-4o-mini"""
...
def _analyze_with_gpt(self, school_name: str, search_results: Dict[str, List]) -> Dict[str, Any]:
    """Analyze search results with GPT-4o-mini - OPTIMIZED FOR COST"""
    ...
    try:
        response = self.openai_client.chat.completions.create(
            model=self.model,  # Now using gpt-4o-mini
            messages=[
                {
                    "role": "system",
        ...
        if hasattr(response, 'usage'):
            ...
            # Much cheaper than GPT-4-turbo: $10/1M input, $30/1M output
        ...
        # FALLBACK: Try one more time with GPT-4o (not mini) for reliability
        logger.warning("Retrying with GPT-4o for better JSON handling")
        try:
            response = self.openai_client.chat.completions.create(
                model="gpt-4o",  # More expensive but more reliable
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert at analyzing search results to extract school information. Always return valid JSON that exactly matches the requested structure."
